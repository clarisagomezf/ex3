...
The problem is that some vegetables appear repeated but with different plots, while the daily harvest is the exact same. This is most likely an error when collecting the plot data. A way to fix it may be eliminating one of the repeated 
...
#is the distinct() thing doing what it is supposed to ?????????????????????????????
```
...
# how do i deleta the scale on the y axis 
```
...
```{r}
Trips %>% 
  mutate(day = wday(edate, label = TRUE)) %>% 
  ggplot(aes(y = day))+
  geom_density()

# is this hows it supposed to look???????????????
```
...
# how do i delete the scale for density????????????????
```
...
Trips %>% 
  mutate(day = wday(edate, label = TRUE)) %>% 
  mutate(weekend = ifelse(day =="Saturday"|day =="Sunday", yes)) %>% 
  mutate(time_day =  hour(sdate)+ minute(sdate)/60) %>% 
  ggplot(aes(x = time_day,fill=client))+
  geom_density(alpha = .5, color=NA)+
...
Trips %>% 
  mutate(day = wday(edate, label = TRUE)) %>% 
  mutate(weekend = ifelse(day =="Saturday"|day =="Sunday", yes)) %>% 
  # i have tto create weekday in the ifelse?????
  mutate(time_day =  hour(sdate)+ minute(sdate)/60) %>% 
  ggplot(aes(x = time_day,fill=weekday))+
  geom_density(alpha = .5, color=NA)+
